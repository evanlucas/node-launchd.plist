...
describe('Plist', function() {
  describe('Verify instance', function() {
    it('Should be an instanceOf Plist', function() {
      var plist = new Plist()
      plist.should.be.an.instanceOf(Plist)
    })
  })
        ...
        describe(test.k+':addObject('+getArg(test.v)+')', function() {
          if (test.err === true) {
            ...
            it('Should not throw an error', function() {
              ...
              plist.obj[test.k].should.be.an.instanceOf(Object)
            })
          }
        })
      })
        ...
        vals.forEach(function(val) {
          describe(s.func+'('+getArg(val.val)+')', function() {
            if (val.err === true) {
              ...
              it('Should not throw an error', function() {
                plist[s.func].call(plist, val.val)
                plist.obj.should.have.property(s.name, Number(val.val)).and.be.type('number')
              })
            }
          })
        })
